...
If the primary agent attempts to use `view_file`, `grep_search`, or similar tools on files named `.env`, `key.properties`, or containing credentials:
1.  **IMMEDIATELY TERMINATE** the process.
2.  **Issue an ERROR**: `[GoalWatch: CRITICAL SECURITY VIOLATION - UNAUTHORIZED SECRET ACCESS ATTEMPTED]`.
3.  Do NOT wait for the output. Block the action.