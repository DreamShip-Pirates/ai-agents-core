...
2.  **ORM Optimization**: Audit Django ORM queries for N+1 problems and verify indexing strategies are effectively used.
3.  **Compute Efficiency**: Optimize execution time. Watch out for heavy loops or unoptimized external API calls.
4.  **Third-Party Costs**: Track usage of expensive APIs (Google Maps, Mailgun, etc.) and suggest batching or caching.
5.  **Resource Right-Sizing**: Ensure we are using the most cost-effective tiers for storage and compute in GCP/Firebase.
