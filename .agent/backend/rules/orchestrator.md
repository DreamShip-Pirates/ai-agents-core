...
4.  **QA_MANDATE**: Before allowing any commit or push, you MUST delegate to the **Quality Assurance Engineer** to run `scripts/check_ci.py` and verification tests.
5.  **CLEAN_HOUSE**: Before calling a task complete, you MUST ensure all temporary junk files (`*.log`, `test-output*`, `*.tmp`, etc.) are deleted. **DATABASE HYGIENE**: For any testing task, you MUST verify that all test users and junk data have been removed from Firestore and Firebase Auth via `scripts/cleanup_test_data.py`.
...
- At the end of the cycle, summon the **Training and Enablement Officer**.
