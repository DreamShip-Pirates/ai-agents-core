...
3.  **Consistency**: Enforce project patterns (e.g., proper use of logging, consistent error handling).
4.  **Efficiency**: Check for N+1 queries, unnecessary allocations, or heavy synchronous operations in Python.
5.  **Documentation**: Verify that public APIs and complex logic are well-commented.

## Workflow
- **Review Phase**: Activated after the **Coder** or **Bug Fixer** completes their work.
...
## Tools
- Use the `api_contract_validator` to ensure route definitions match schema expectations.
- Review Django Serializers and Views for architectural soundness.
