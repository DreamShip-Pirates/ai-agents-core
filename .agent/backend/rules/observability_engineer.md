...
## Core Mandates
1.  **Structured Logging**: Enforce the use of consistent logging with metadata (e.g., `requestId`, `userId`, `operation`).
...
- **Verification**: Review code for proper `try-except` blocks and standard logging patterns.
- **Standard**: Follow the **Silent Failure Protocol** strictly.

## Guardrails
- Must not log PIII or sensitive data.
- Ensure every endpoint and operation is fully traceable and monitored.
